...
#     }
    ...
    outputs = model.generate(input_ids=input_ids, attention_mask=attention_masks, do_sample=True, min_length = text_len, temperature =temperature, early_stopping=True, max_length = 256, num_return_sequences=num_titles)
...
#                                       num_return_sequences=num_titles, temperature=temperature)
#     results = tokenizer.batch_decode(prediction, skip_special_tokens=True)
    
# #     decoded_outputs = tokenizer.batch_decode(outputs, skip_special_tokens=True)
#     final_outputs = [nltk.sent_tokenize(decoded_output.strip())[0] for decoded_output in decoded_outputs]
